...
def text_detect(input, ele_size=(8, 2)):
    ...
    if len(input.shape) == 3:
        ...
        cv2.imwrite("image_samples/1.png", test)
    # inverts colors
    img_sobel = cv2.Scharr(test, cv2.CV_8U, 1, 0)
    #img_sobel2 = cv2.Sobel(test, cv2.CV_8U, 0, 1)
    #cv2.imwrite("image_samples/2.png", img_sobel)
    #cv2.imwrite("image_samples/3.png", img_sobel2)
...
def main(input_file):
    outputFile = input_file.split('.')[0] + '-detected.' + '.'.join(input_file.split('.')[1:])
    print(outputFile)
...
if __name__ == '__main__':
    main("image_samples/step1_image_colored.png")
